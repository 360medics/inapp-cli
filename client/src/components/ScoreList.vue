<template>
    <ul class="scoreList">
        <li v-for="list in scores" :key="list.id" class="scoreList__items">
            <span v-if="list.submenu" class="scoreList__item" @click="onToggle=!onToggle; itemSelected(list.id)">
                {{ list.name }}
                <i class="scoreList__item__icon fas" :class="`${onToggle && list.id === optionSelectedID ? 'fa-chevron-up' : 'fa-chevron-down'}`" />
            </span>

            <div :id="list.id" class="submenu" :class="`${onToggle && list.id === optionSelectedID ? '' : 'submenu--hidden'}`">
...
export default defineComponent({
    ...
    setup() {
        ...
        const itemSelected = (item: number) :void => { state.optionSelectedID = item }
